...
void List_Dispose( List *list ) {
    ...
    ListElementPtr curr = list->firstElement;
    ListElementPtr next = NULL;

    //prejde list a postupne uvolnuje elementy 
    while(curr != NULL)
    {
    ...
    // na koniec nastavi pointre ako NULL kedze je neinicializovany 
    list->activeElement = NULL;
    list->firstElement = NULL;
...
void List_InsertFirst( List *list, int data ) {
    ListElementPtr curr = malloc(sizeof(struct ListElement));
    if(curr == NULL)  // kontrola ci presiel malloc 
    {
        List_Error();
        return;
    }
...
void List_DeleteFirst( List *list ) {
    // kontrola ci je list neprazdny 
    if(list->firstElement == NULL)
        return;
    // kontrola ci nie je nahodou aktivny prvy element ak ano nastavi sa list ako neaktivny 
    if(list->firstElement == list->activeElement)
    {
        list->activeElement = NULL;
    }
    ListElementPtr first = list->firstElement;  // pomocna premenna
    list->firstElement=list->firstElement->nextElement;// nastavenie dat pomocou pomocnej premennej
    free(first);// uvolnenie alokovaneho miesta
...
void List_DeleteAfter( List *list ) {
    // kontrola ci je list aktivny a ci nie je element posledny 
    if(list->activeElement == NULL || list->activeElement->nextElement == NULL)
        return; 

    ListElementPtr todelete = list->activeElement->nextElement; // pomocna premenna 
    list->activeElement->nextElement = todelete->nextElement; // prepisanie dat
    free(todelete); // uvolnenie alokovanej pamete
...
void List_InsertAfter( List *list, int data ) {
    // kontrola ci je list aktivny 
    if(list->activeElement == NULL)
        return; 
    ListElementPtr new = malloc(sizeof(struct ListElement));
    if(new == NULL)  // kontrola ci uspel malloc 
    {
        List_Error();
        return;
    }
...
void List_Next( List *list ) {
    ...
    // ak je aktivny nastavy sa nasledujuci prvok ako aktivny 
...
int List_IsActive( List *list ) {
    //ak je active element NULL pointer list nie je aktivny
